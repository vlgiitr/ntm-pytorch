...
def generate_data(seq_width, min_seq_len, max_seq_len, batch_size, num_batches):
    """Generates random sequences of random vectors for copy task.
       To account for the delimiter flag, the input sequence length as well width is one more than the target sequence.
       Returns
       -------
       An array with each element representing a batch. Each element is a tuple containing the batch number, input and target tensors.
    ...
    for batch_num in range(num_batches):
        ...
        input = Variable(torch.zeros(batch_size, seq_len + 1, seq_width + 1))
